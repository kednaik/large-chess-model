...
VOCAB_V1 = {
    ...
    '[EP_A]': 23, '[EP_B]': 24, '[EP_C]': 25, '[EP_D]': 26, 
    '[EP_E]': 27, '[EP_F]': 28, '[EP_G]': 29, '[EP_H]': 30, '[EP_NONE]': 31
}

class ChessTransformer(nn.Module):
    def __init__(self, vocab_size=len(VOCAB_V1)+1, d_model=256, nhead=8, num_layers=6, dim_feedforward=1024, dropout=0.1):
        super().__init__()
        self.embedding = nn.Embedding(vocab_size, d_model)
        self.pos_encoding = nn.Parameter(torch.zeros(1, 71, d_model))
    ...
    def forward(self, x):
        ...
        return self.fc_out(x[:, 7:, :])

def encode_state_v1(board, win_token):
    seq = [win_token]
    if board.fullmove_number == 1 and board.turn == chess.WHITE:
        seq.append(VOCAB_V1['[INIT]'])
    else:
        seq.append(VOCAB_V1['[W_MOVE]'] if board.turn == chess.WHITE else VOCAB_V1['[B_MOVE]'])
    seq.append(VOCAB_V1['[W_KSC]'] if board.has_kingside_castling_rights(chess.WHITE) else 0)
    seq.append(VOCAB_V1['[W_QSC]'] if board.has_queenside_castling_rights(chess.WHITE) else 0)
    seq.append(VOCAB_V1['[B_KSC]'] if board.has_kingside_castling_rights(chess.BLACK) else 0)
    seq.append(VOCAB_V1['[B_QSC]'] if board.has_queenside_castling_rights(chess.BLACK) else 0)
    ep = board.ep_square
    seq.append(VOCAB_V1[f'[EP_{chess.square_name(ep)[0].upper()}]'] if ep else VOCAB_V1['[EP_NONE]'])
    for s in chess.SQUARES:
        p = board.piece_at(s)
        seq.append(VOCAB_V1[p.symbol()] if p else VOCAB_V1['EMPTY'])
    return seq

def get_best_move_v1(model, board, device):
    model.eval()
    win_token = VOCAB_V1['[W_WIN]'] if board.turn == chess.WHITE else VOCAB_V1['[B_WIN]']
    input_seq = encode_state_v1(board, win_token)
    ...
    for move in board.legal_moves:
        ...
        for s in chess.SQUARES:
            p = board.piece_at(s)
            target_token = VOCAB_V1[p.symbol()] if p else VOCAB_V1['EMPTY']
...
# Load V1
model_v1 = ChessTransformer().to(device)
v1_path = "../checkpoints/run_20260312_002355/chess_model_best.pt"
try:
    model_v1.load_state_dict(torch.load(v1_path, map_location=device, weights_only=True))
    model_v1.eval()
    print("V1 Model loaded successfully!")
except Exception as e:
    print(f"FAILED to load V1 from {v1_path}. Error: {e}")

# Load ViT-Dual
model_vit = ViTChessHybrid(d_model=256, nhead=8, num_layers=8, dropout=0.1).to(device)
vit_path = "../checkpoints/run_20260313_133630/chess_vit_best.pt"
try:
    ckpt = torch.load(vit_path, map_location=device, weights_only=True)
    if isinstance(ckpt, dict) and 'model' in ckpt:
        model_vit.load_state_dict(ckpt['model'])
    else:
        model_vit.load_state_dict(ckpt)
    model_vit.eval()
    print("ViT-Dual Model loaded successfully!")
except Exception as e:
    print(f"FAILED to load ViT-Dual from {vit_path}. Error: {e}")
...
def predict_move(request: MoveRequest):
    ...
    if request.modelVersion == "vit":
        best_move = get_best_move_vit(model_vit, board, device, request.whiteElo, request.blackElo)
    else:
        best_move = get_best_move_v1(model_v1, board, device)
